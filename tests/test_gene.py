...
def test_gene_taxon(gene_information_entities):
    gene = gene_information_entities[0]
    assert gene.in_taxon == ["NCBITaxon:4896"]
    assert gene.in_taxon_label == "Schizosaccharomyces pombe"


def test_gene_provided_by(gene_information_entities):
    gene = gene_information_entities[0]
    assert gene.provided_by == ["infores:pombase"]
